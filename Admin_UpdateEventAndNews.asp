...
If (CStr(Request("MM_update")) = "form1") Then
  If (Not MM_abortEdit) Then
    ...
    MM_editCmd.CommandText = "UPDATE dbo.tbEventAndNews SET EventSummary = ?, EventContent = ?, EventDate = ?, EventDuration = ?, EventImage = ? WHERE EventID = ?" 
    MM_editCmd.Prepared = true
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param1", 202, 1, 100, Request.Form("EventSummary")) ' adVarWChar
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param2", 202, 1, -1, Request.Form("EventContent")) ' adVarWChar
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param3", 135, 1, -1, MM_IIF(Request.Form("EventDate"), Request.Form("EventDate"), null)) ' adDBTimeStamp
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param4", 202, 1, 100, Request.Form("EventDuration")) ' adVarWChar
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param5", 201, 1, 200, Request.Form("EventImage")) ' adLongVarChar
...
%>
<%
Dim rsUpdateEventAndNews__MMColParam
rsUpdateEventAndNews__MMColParam = "1"
If (Request.QueryString("EventID") <> "") Then 
  rsUpdateEventAndNews__MMColParam = Request.QueryString("EventID")
End If
%>
<%
Dim rsUpdateEventAndNews
Dim rsUpdateEventAndNews_cmd
Dim rsUpdateEventAndNews_numRows

Set rsUpdateEventAndNews_cmd = Server.CreateObject ("ADODB.Command")
rsUpdateEventAndNews_cmd.ActiveConnection = MM_cn_STRING
rsUpdateEventAndNews_cmd.CommandText = "SELECT * FROM dbo.tbEventAndNews WHERE EventID = ?" 
rsUpdateEventAndNews_cmd.Prepared = true
rsUpdateEventAndNews_cmd.Parameters.Append rsUpdateEventAndNews_cmd.CreateParameter("param1", 5, 1, -1, rsUpdateEventAndNews__MMColParam) ' adDouble

Set rsUpdateEventAndNews = rsUpdateEventAndNews_cmd.Execute
rsUpdateEventAndNews_numRows = 0
%>
<%
Dim rsUpdate__MMColParam
rsUpdate__MMColParam = "1"
If (Request.QueryString("EventID") <> "") Then 
  rsUpdate__MMColParam = Request.QueryString("EventID")
End If
%>
<%
Dim rsUpdate
...
rsUpdate_cmd.CommandText = "SELECT * FROM dbo.tbEventAndNews WHERE EventID = ?" 
...
Repeat2__numRows = 10
Repeat2__index = 0
rsUpdateEventAndNews_numRows = rsUpdateEventAndNews_numRows + Repeat2__numRows
%>
<%
Dim MM_paramName 
%>
...
function cut_string()
{
	...
	document.getElementById("EventImage").value = name;
	document.getElementById("fieldImage").src = "images\\event_news\\" + name;
}
    ...
    <div class="row">
      ...
      <div class="col-sm-9 padding-right">
        <!-- InstanceBeginEditable name="Content" -->
                    <h2 class="title text-center">Cập Nhật Tin Tức &amp; Sự Kiện</h2>
                    <div class="col-sm-12">
                    <form ACTION="<%=MM_editAction%>" id="form1" name="form1" method="POST" onSubmit="return check()">
                   	  <table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" bordercolor="#FFFFFF">
                            <% While ((Repeat2__numRows <> 0) AND (NOT rsUpdateEventAndNews.EOF)) %>
                            <tr>
                            ...
                            <tr>
                                <td align="right" valign="top">&nbsp;</td>
                                <td align="left" valign="top"><input id="EventDate" name="EventDate" type="hidden" size="32" value="<%=(rsUpdateEventAndNews.Fields.Item("EventDate").Value)%>" readonly/></td>
                            </tr>
                            <% 
  								Repeat2__index=Repeat2__index+1
  								Repeat2__numRows=Repeat2__numRows-1
  								rsUpdateEventAndNews.MoveNext()
								Wend
							%>
   					  </table>
                      <input type="hidden" name="MM_update" value="form1">
                      <input type="hidden" name="MM_recordId" value="<%= rsUpdate.Fields.Item("EventID").Value %>">
                    </form>
                    </div>
					<!-- InstanceEndEditable -->
      </div>
...
%>
<%
rsUpdateEventAndNews.Close()
Set rsUpdateEventAndNews = Nothing
%>
<%
rsUpdate.Close()
