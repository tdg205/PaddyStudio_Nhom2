...
<%
Dim rsManageUser
Dim rsManageUser_cmd
Dim rsManageUser_numRows

Set rsManageUser_cmd = Server.CreateObject ("ADODB.Command")
rsManageUser_cmd.ActiveConnection = MM_cn_STRING
rsManageUser_cmd.CommandText = "SELECT * FROM dbo.tbUser" 
rsManageUser_cmd.Prepared = true

Set rsManageUser = rsManageUser_cmd.Execute
rsManageUser_numRows = 0
%>
<%
...
Repeat2__numRows = 5
Repeat2__index = 0
rsManageUser_numRows = rsManageUser_numRows + Repeat2__numRows
%>
<%
'  *** Recordset Stats, Move To Record, and Go To Record: declare stats variables

Dim rsManageUser_total
Dim rsManageUser_first
Dim rsManageUser_last

' set the record count
rsManageUser_total = rsManageUser.RecordCount

' set the number of rows displayed on this page
If (rsManageUser_numRows < 0) Then
  rsManageUser_numRows = rsManageUser_total
Elseif (rsManageUser_numRows = 0) Then
  rsManageUser_numRows = 1
End If

' set the first and last displayed record
rsManageUser_first = 1
rsManageUser_last  = rsManageUser_first + rsManageUser_numRows - 1

' if we have the correct record count, check the other stats
If (rsManageUser_total <> -1) Then
  If (rsManageUser_first > rsManageUser_total) Then
    rsManageUser_first = rsManageUser_total
  End If
  If (rsManageUser_last > rsManageUser_total) Then
    rsManageUser_last = rsManageUser_total
  End If
  If (rsManageUser_numRows > rsManageUser_total) Then
    rsManageUser_numRows = rsManageUser_total
  End If
End If
%>
...
Set MM_rs    = rsManageUser
MM_rsCount   = rsManageUser_total
MM_size      = rsManageUser_numRows
MM_uniqueCol = ""
MM_paramName = ""
MM_offset = 0
MM_atTotal = false
...
' set the first and last displayed record
rsManageUser_first = MM_offset + 1
rsManageUser_last  = MM_offset + MM_size

If (MM_rsCount <> -1) Then
  If (rsManageUser_first > MM_rsCount) Then
    rsManageUser_first = MM_rsCount
  End If
  If (rsManageUser_last > MM_rsCount) Then
    rsManageUser_last = MM_rsCount
  End If
End If
				...
				<div class="col-sm-9 padding-right">
                    ...
                    <div class="col-sm-12">
                    	<table width="100%" border="1" cellpadding="5" cellspacing="5">
                  			...
                  			<% While ((Repeat2__numRows <> 0) AND (NOT rsManageUser.EOF)) %>
                    		<tr>
                      			<td><%=(rsManageUser.Fields.Item("UserID").Value)%></td>
                      			<td><%=(rsManageUser.Fields.Item("FullName").Value)%></td>
                      			<td><a HREF="Admin_DetailUser.asp?<%= Server.HTMLEncode(MM_keepNone) & MM_joinChar(MM_keepNone) & "UserID=" & rsManageUser.Fields.Item("UserID").Value %>" class="btn search">Xem</a></td>
                    		</tr>
                    		<% 
  								Repeat2__index=Repeat2__index+1
  								Repeat2__numRows=Repeat2__numRows-1
  								rsManageUser.MoveNext()
								Wend
							%>
                		</table>
                        <br/>
