...
If(Session("newEmail") <> Session("oldEmail")) Then
	...
	If (CStr(Request(MM_flag)) <> "") Then
  		...
  		MM_dupKeyRedirect = "User_AccountUpdateFail.asp"
...
If (CStr(Request("MM_update")) = "form1") Then
  If (Not MM_abortEdit) Then
    ...
    MM_editRedirectUrl = "User_Account.asp"
    If (Request.QueryString <> "") Then
...
%>
<%
Dim rsAccountUpdate__MMColParam
rsAccountUpdate__MMColParam = "1"
If (Request.QueryString("UserID") <> "") Then 
	rsAccountUpdate__MMColParam = Request.QueryString("UserID")
Else 
	rsAccountUpdate__MMColParam = Session("MM_Username")
End If
%>
<%
Dim rsAccountUpdate
Dim rsAccountUpdate_cmd
Dim rsAccountUpdate_numRows

Set rsAccountUpdate_cmd = Server.CreateObject ("ADODB.Command")
rsAccountUpdate_cmd.ActiveConnection = MM_cn_STRING
rsAccountUpdate_cmd.CommandText = "SELECT * FROM dbo.tbUser WHERE UserID = ?" 
rsAccountUpdate_cmd.Prepared = true
rsAccountUpdate_cmd.Parameters.Append rsAccountUpdate_cmd.CreateParameter("param1", 200, 1, 20, rsAccountUpdate__MMColParam) ' adVarChar

Set rsAccountUpdate = rsAccountUpdate_cmd.Execute
rsAccountUpdate_numRows = 0
%>
<%
...
%>
<%
rsAccountUpdate.Close()
Set rsAccountUpdate = Nothing
%>
