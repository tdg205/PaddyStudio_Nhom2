...
<%
Dim rsManageProduct
Dim rsManageProduct_cmd
Dim rsManageProduct_numRows

Set rsManageProduct_cmd = Server.CreateObject ("ADODB.Command")
rsManageProduct_cmd.ActiveConnection = MM_cn_STRING
rsManageProduct_cmd.CommandText = "SELECT ProductID, ProductName, ProductImage FROM dbo.tbProduct ORDER BY ProductID DESC" 
rsManageProduct_cmd.Prepared = true

Set rsManageProduct = rsManageProduct_cmd.Execute
rsManageProduct_numRows = 0
%>
<%
...
Repeat2__numRows = 5
Repeat2__index = 0
rsManageProduct_numRows = rsManageProduct_numRows + Repeat2__numRows
%>
<%
'  *** Recordset Stats, Move To Record, and Go To Record: declare stats variables

Dim rsManageProduct_total
Dim rsManageProduct_first
Dim rsManageProduct_last

' set the record count
rsManageProduct_total = rsManageProduct.RecordCount

' set the number of rows displayed on this page
If (rsManageProduct_numRows < 0) Then
  rsManageProduct_numRows = rsManageProduct_total
Elseif (rsManageProduct_numRows = 0) Then
  rsManageProduct_numRows = 1
End If

' set the first and last displayed record
rsManageProduct_first = 1
rsManageProduct_last  = rsManageProduct_first + rsManageProduct_numRows - 1

' if we have the correct record count, check the other stats
If (rsManageProduct_total <> -1) Then
  If (rsManageProduct_first > rsManageProduct_total) Then
    rsManageProduct_first = rsManageProduct_total
  End If
  If (rsManageProduct_last > rsManageProduct_total) Then
    rsManageProduct_last = rsManageProduct_total
  End If
  If (rsManageProduct_numRows > rsManageProduct_total) Then
    rsManageProduct_numRows = rsManageProduct_total
  End If
End If
%>
...
Set MM_rs    = rsManageProduct
MM_rsCount   = rsManageProduct_total
MM_size      = rsManageProduct_numRows
MM_uniqueCol = ""
MM_paramName = ""
MM_offset = 0
MM_atTotal = false
...
' set the first and last displayed record
rsManageProduct_first = MM_offset + 1
rsManageProduct_last  = MM_offset + MM_size

If (MM_rsCount <> -1) Then
  If (rsManageProduct_first > MM_rsCount) Then
    rsManageProduct_first = MM_rsCount
  End If
  If (rsManageProduct_last > MM_rsCount) Then
    rsManageProduct_last = MM_rsCount
  End If
End If
...
%>
<%
rsManageProduct.Close()
Set rsManageProduct = Nothing
%>
