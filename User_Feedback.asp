...
rsFeedbackList_cmd.Parameters.Append rsFeedbackList_cmd.CreateParameter("param1", 200, 1, 20, rsFeedbackList__MMColParam) ' adVarChar

Set rsFeedbackList = rsFeedbackList_cmd.Execute
rsFeedbackList_numRows = 0
%>
<%
...
Repeat2__numRows = 5
Repeat2__index = 0
rsFeedbackList_numRows = rsFeedbackList_numRows + Repeat2__numRows
%>
<%
...
rsFeedbackList_total = rsFeedbackList.RecordCount

' set the number of rows displayed on this page
If (rsFeedbackList_numRows < 0) Then
  rsFeedbackList_numRows = rsFeedbackList_total
Elseif (rsFeedbackList_numRows = 0) Then
  rsFeedbackList_numRows = 1
End If

' set the first and last displayed record
rsFeedbackList_first = 1
rsFeedbackList_last  = rsFeedbackList_first + rsFeedbackList_numRows - 1

' if we have the correct record count, check the other stats
If (rsFeedbackList_total <> -1) Then
  If (rsFeedbackList_first > rsFeedbackList_total) Then
    rsFeedbackList_first = rsFeedbackList_total
  End If
  If (rsFeedbackList_last > rsFeedbackList_total) Then
    rsFeedbackList_last = rsFeedbackList_total
  End If
  If (rsFeedbackList_numRows > rsFeedbackList_total) Then
    rsFeedbackList_numRows = rsFeedbackList_total
  End If
End If
%>
...
Set MM_rs    = rsFeedbackList
MM_rsCount   = rsFeedbackList_total
MM_size      = rsFeedbackList_numRows
MM_uniqueCol = ""
MM_paramName = ""
MM_offset = 0
MM_atTotal = false
...
If (MM_rsCount <> -1) Then
  If (rsFeedbackList_first > MM_rsCount) Then
    rsFeedbackList_first = MM_rsCount
  End If
  If (rsFeedbackList_last > MM_rsCount) Then
    rsFeedbackList_last = MM_rsCount
  End If
End If
		...
		<div class="container">
			<div class="row">
				...
				<div class="col-sm-9 padding-right">
                    ...
                    <h2 class="title text-center">Danh Sách Phản Hồi</h2>
                    <div class="col-sm-12">
                   		<table width="100%" border="1" cellpadding="5" cellspacing="5">
                            ...
                            <% While ((Repeat2__numRows <> 0) AND (NOT rsFeedbackList.EOF)) %>
  							<tr>
    							<td><%=(rsFeedbackList.Fields.Item("FeedbackSubject").Value)%></td>
    							<td><%=(rsFeedbackList.Fields.Item("FeedbackSummary").Value)%></td>
                                <td><%=(rsFeedbackList.Fields.Item("FeedbackDate").Value)%></td>
    							<td><a HREF="User_FeedbackDetail.asp?<%= Server.HTMLEncode(MM_keepNone) & MM_joinChar(MM_keepNone) & "FeedbackID=" & rsFeedbackList.Fields.Item("FeedbackID").Value %>" class="btn search">Xem</a></td>
  							</tr>
  							<% 
  								Repeat2__index=Repeat2__index+1
  								Repeat2__numRows=Repeat2__numRows-1
  								rsFeedbackList.MoveNext()
								Wend
							%>
                        </table>
                        <br/>
...
%>
<%
rsFeedbackList.Close()
Set rsFeedbackList = Nothing
%>
<%
rsFeedbackID.Close()
