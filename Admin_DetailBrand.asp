...
If (CStr(Request("MM_delete")) = "form1" And CStr(Request("MM_recordId")) <> "") Then
  ...
  If (Not MM_abortEdit) Then
    ...
    MM_editCmd.CommandText = "DELETE FROM dbo.tbBrand WHERE BrandName = ?"
...
%>
<%
Dim rsBrandDetail__MMColParam
rsBrandDetail__MMColParam = "1"
If (Request.QueryString("BrandName") <> "") Then 
  rsBrandDetail__MMColParam = Request.QueryString("BrandName")
End If
%>
<%
Dim rsBrandDetail
Dim rsBrandDetail_cmd
Dim rsBrandDetail_numRows

Set rsBrandDetail_cmd = Server.CreateObject ("ADODB.Command")
rsBrandDetail_cmd.ActiveConnection = MM_cn_STRING
rsBrandDetail_cmd.CommandText = "SELECT * FROM dbo.tbBrand WHERE BrandName = ?" 
rsBrandDetail_cmd.Prepared = true
rsBrandDetail_cmd.Parameters.Append rsBrandDetail_cmd.CreateParameter("param1", 200, 1, 20, rsBrandDetail__MMColParam) ' adVarChar

Set rsBrandDetail = rsBrandDetail_cmd.Execute
rsBrandDetail_numRows = 0
%>
<%
Dim rsXoa__MMColParam
rsXoa__MMColParam = "1"
If (Request.QueryString("BrandName") <> "") Then 
  rsXoa__MMColParam = Request.QueryString("BrandName")
End If
%>
<%
Dim rsXoa
...
rsXoa_cmd.CommandText = "SELECT * FROM dbo.tbBrand WHERE BrandName = ?" 
...
%>
<%
Dim rsPro__MMColParam
rsPro__MMColParam = "1"
If (Request.QueryString("BrandName") <> "") Then 
  rsPro__MMColParam = Request.QueryString("BrandName")
End If
%>
<%
Dim rsPro
...
Set rsPro_cmd = Server.CreateObject ("ADODB.Command")
rsPro_cmd.ActiveConnection = MM_cn_STRING
rsPro_cmd.CommandText = "SELECT * FROM dbo.tbProduct WHERE BrandName = ?" 
rsPro_cmd.Prepared = true
rsPro_cmd.Parameters.Append rsPro_cmd.CreateParameter("param1", 200, 1, 20, rsPro__MMColParam) ' adVarChar

Set rsPro = rsPro_cmd.Execute
rsPro_numRows = 0
%>
<%
    ...
    <div class="row">
      ...
      <div class="col-sm-9 padding-right">
        <!-- InstanceBeginEditable name="Content" -->
                    <h2 class="title text-center">Xem Thương Hiệu</h2>
                    <div class="col-sm-12">
                    <form ACTION="<%=MM_editAction%>"  id="form1" name="form1" method="POST">
					<%  If((Not rsBrandDetail.BOF) And (Not rsBrandDetail.EOF)) Then %>
                   	  <table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" bordercolor="#FFFFFF">
                            <tr>
          						<td colspan="2" align="center" valign="middle"><img src="images/logo/<%=(rsBrandDetail.Fields.Item("BrandLogo").Value)%>" width="300" height="300"></td>
       						</tr>
        					<tr>
          						<td width="40%" align="right" valign="top"><strong>Tên Thương Hiệu: &nbsp;</strong></td>
          						<td width="60%" align="left" valign="top"><%=(rsBrandDetail.Fields.Item("BrandName").Value)%></td>
       						</tr>
                            <tr>
                                <td align="right" valign="top"><strong>Mô Tả: &nbsp;</strong></td>
                              <td align="left" valign="top"><%=(rsBrandDetail.Fields.Item("BrandDescription").Value)%></td>
                            </tr>
                            <tr>
                      ...
                      <input type="hidden" name="MM_recordId" value="<%= rsXoa.Fields.Item("BrandName").Value %>">
					<% Else %>
						<h3 class="title text-center">Bạn hãy chọn lại một Thương Hiệu cần xem chi tiết.</h3>
					<% End If %>
                    </form>
                    </div>
                    <!-- InstanceEndEditable -->
