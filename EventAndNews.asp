...
<%
Dim rsEventAndNews
Dim rsEventAndNews_cmd
Dim rsEventAndNews_numRows

Set rsEventAndNews_cmd = Server.CreateObject ("ADODB.Command")
rsEventAndNews_cmd.ActiveConnection = MM_cn_STRING
rsEventAndNews_cmd.CommandText = "SELECT *, LEFT(EventContent,500)+'...' as 'BriefDescription' FROM dbo.tbEventAndNews ORDER BY EventDate DESC" 
rsEventAndNews_cmd.Prepared = true

Set rsEventAndNews = rsEventAndNews_cmd.Execute
rsEventAndNews_numRows = 0
%>
<%
...
Repeat2__numRows = 3
Repeat2__index = 0
rsEventAndNews_numRows = rsEventAndNews_numRows + Repeat2__numRows
%>
<%
'  *** Recordset Stats, Move To Record, and Go To Record: declare stats variables

Dim rsEventAndNews_total
Dim rsEventAndNews_first
Dim rsEventAndNews_last

' set the record count
rsEventAndNews_total = rsEventAndNews.RecordCount

' set the number of rows displayed on this page
If (rsEventAndNews_numRows < 0) Then
  rsEventAndNews_numRows = rsEventAndNews_total
Elseif (rsEventAndNews_numRows = 0) Then
  rsEventAndNews_numRows = 1
End If

' set the first and last displayed record
rsEventAndNews_first = 1
rsEventAndNews_last  = rsEventAndNews_first + rsEventAndNews_numRows - 1

' if we have the correct record count, check the other stats
If (rsEventAndNews_total <> -1) Then
  If (rsEventAndNews_first > rsEventAndNews_total) Then
    rsEventAndNews_first = rsEventAndNews_total
  End If
  If (rsEventAndNews_last > rsEventAndNews_total) Then
    rsEventAndNews_last = rsEventAndNews_total
  End If
  If (rsEventAndNews_numRows > rsEventAndNews_total) Then
    rsEventAndNews_numRows = rsEventAndNews_total
  End If
End If
%>
...
Set MM_rs    = rsEventAndNews
MM_rsCount   = rsEventAndNews_total
MM_size      = rsEventAndNews_numRows
MM_uniqueCol = ""
MM_paramName = ""
MM_offset = 0
MM_atTotal = false
...
' set the first and last displayed record
rsEventAndNews_first = MM_offset + 1
rsEventAndNews_last  = MM_offset + MM_size

If (MM_rsCount <> -1) Then
  If (rsEventAndNews_first > MM_rsCount) Then
    rsEventAndNews_first = MM_rsCount
  End If
  If (rsEventAndNews_last > MM_rsCount) Then
    rsEventAndNews_last = MM_rsCount
  End If
End If
				...
				<div class="col-sm-9 padding-right">
                    ...
                    <div class="col-sm-12">
						...
						<div class="single-blog-post">
							...
							<p align="justify"><img src="images/event_news/<%=(rsEventAndNews.Fields.Item("EventImage").Value)%>"></p>
							<p align="justify"><%=(rsEventAndNews.Fields.Item("BriefDescription").Value)%></p>
							<div align="right"><A HREF="EventAndNews_Detail.asp?<%= Server.HTMLEncode(MM_keepNone) & MM_joinChar(MM_keepNone) & "EventID=" & rsEventAndNews.Fields.Item("EventID").Value %>" class="btn btn-primary">Xem Chi Tiết</A></div>
                            <hr>
						</div>
                    	<% 
  							Repeat2__index=Repeat2__index+1
  							Repeat2__numRows=Repeat2__numRows-1
  							rsEventAndNews.MoveNext()
							Wend
						%>
					</div>
                    <div class="col-sm-12">
...
%>
<%
rsEventAndNews.Close()
Set rsEventAndNews = Nothing
%>
