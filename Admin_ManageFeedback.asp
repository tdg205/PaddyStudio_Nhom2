...
%>
<%
Dim rsAdminFeedbackList
Dim rsAdminFeedbackList_cmd
Dim rsAdminFeedbackList_numRows

Set rsAdminFeedbackList_cmd = Server.CreateObject ("ADODB.Command")
rsAdminFeedbackList_cmd.ActiveConnection = MM_cn_STRING
rsAdminFeedbackList_cmd.CommandText = "SELECT *, left(FeedbackContent, 50)+'...' as 'FeedbackSummary' FROM dbo.tbFeedback ORDER BY case when FeedbackReply is null then 0 else 1 end, FeedbackID DESC" 
rsAdminFeedbackList_cmd.Prepared = true

Set rsAdminFeedbackList = rsAdminFeedbackList_cmd.Execute
rsAdminFeedbackList_numRows = 0
%>
<%
...
Repeat2__numRows = 5
Repeat2__index = 0
rsAdminFeedbackList_numRows = rsAdminFeedbackList_numRows + Repeat2__numRows
%>
<%
'  *** Recordset Stats, Move To Record, and Go To Record: declare stats variables

Dim rsAdminFeedbackList_total
Dim rsAdminFeedbackList_first
Dim rsAdminFeedbackList_last

' set the record count
rsAdminFeedbackList_total = rsAdminFeedbackList.RecordCount

' set the number of rows displayed on this page
If (rsAdminFeedbackList_numRows < 0) Then
  rsAdminFeedbackList_numRows = rsAdminFeedbackList_total
Elseif (rsAdminFeedbackList_numRows = 0) Then
  rsAdminFeedbackList_numRows = 1
End If

' set the first and last displayed record
rsAdminFeedbackList_first = 1
rsAdminFeedbackList_last  = rsAdminFeedbackList_first + rsAdminFeedbackList_numRows - 1

' if we have the correct record count, check the other stats
If (rsAdminFeedbackList_total <> -1) Then
  If (rsAdminFeedbackList_first > rsAdminFeedbackList_total) Then
    rsAdminFeedbackList_first = rsAdminFeedbackList_total
  End If
  If (rsAdminFeedbackList_last > rsAdminFeedbackList_total) Then
    rsAdminFeedbackList_last = rsAdminFeedbackList_total
  End If
  If (rsAdminFeedbackList_numRows > rsAdminFeedbackList_total) Then
    rsAdminFeedbackList_numRows = rsAdminFeedbackList_total
  End If
End If
%>
...
Set MM_rs    = rsAdminFeedbackList
MM_rsCount   = rsAdminFeedbackList_total
MM_size      = rsAdminFeedbackList_numRows
MM_uniqueCol = ""
MM_paramName = ""
MM_offset = 0
MM_atTotal = false
...
' set the first and last displayed record
rsAdminFeedbackList_first = MM_offset + 1
rsAdminFeedbackList_last  = MM_offset + MM_size

If (MM_rsCount <> -1) Then
  If (rsAdminFeedbackList_first > MM_rsCount) Then
    rsAdminFeedbackList_first = MM_rsCount
  End If
  If (rsAdminFeedbackList_last > MM_rsCount) Then
    rsAdminFeedbackList_last = MM_rsCount
  End If
End If
    ...
    <div class="row">
      ...
      <div class="col-sm-9 padding-right">
        <!-- InstanceBeginEditable name="Content" -->
                    <h2 class="title text-center">Quản Lý Phản Hồi</h2>
                    <div class="col-sm-12">
                        ...
                        <% While ((Repeat2__numRows <> 0) AND (NOT rsAdminFeedbackList.EOF)) %>
  						<tr>
    						<td><%=(rsAdminFeedbackList.Fields.Item("FeedbackMemberID").Value)%></td>
    						<td><%=(rsAdminFeedbackList.Fields.Item("FeedbackSubject").Value)%></td>
    						<td><%=(rsAdminFeedbackList.Fields.Item("FeedbackSummary").Value)%></td>
    						<td><%=(rsAdminFeedbackList.Fields.Item("FeedbackDate").Value)%></td>
    						<td><a HREF="Admin_DetailFeedback.asp?<%= Server.HTMLEncode(MM_keepNone) & MM_joinChar(MM_keepNone) & "FeedbackID=" & rsAdminFeedbackList.Fields.Item("FeedbackID").Value %>" class="btn search">Xem</a></td>
                            <td><a HREF="Admin_UpdateFeedback.asp?<%= Server.HTMLEncode(MM_keepNone) & MM_joinChar(MM_keepNone) & "FeedbackID=" & rsAdminFeedbackList.Fields.Item("FeedbackID").Value %>" class="btn search">Trả Lời</a></td>
  						</tr>
    					<% 
    						Repeat2__index=Repeat2__index+1
    						Repeat2__numRows=Repeat2__numRows-1
    						rsAdminFeedbackList.MoveNext()
    						Wend
						%>
                      </table>
                      	<br/>
...
%>
<%
rsAdminFeedbackList.Close()
Set rsAdminFeedbackList = Nothing
%>
<%
rsFeedbackID.Close()
